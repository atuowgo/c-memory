...
1. **打包**：在 `c-memory` 仓库自己的目录下执行 `build.sh`，产出 `dist/`。
2. **部署**：把整个 `dist/` 目录当**一个自包含子目录**（比如改名成 `c-memory/`）放进目标项目根目录，在这个子目录里执行 `install.sh`。
...
产出 `dist/c-memory/`（`dist/` 只是构建产物的落脚目录，`c-memory/` 这一层才是真正自包含的部署单元，包名固定，不用用户手动加），内容清单：
...
子目录名默认叫 `c-memory`（跟 `dist/` 下的目录名一致），如果手动改了名字，`install.sh` 会自动识别自己所在的目录名（`basename`）并用作 hook 命令的路径前缀，改名后也能正常工作，不需要额外配置。**唯一约束**：这个子目录必须是目标项目**根目录的直接子目录**（不能再嵌套一层），因为 `.claude/settings.json` 只会往上找一级。
