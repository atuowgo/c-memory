...
然后在项目根目录配置 `.env`（完整结构见 `.env.example`）：
...
`LLM_PROVIDER` 用于在 `deepseek`/`openai`/`anthropic` 三个 Provider 间显式选型；每个 Provider 都有 `{PREFIX}_API_KEY`/`{PREFIX}_BASE_URL`/`{PREFIX}_MODEL` 三个变量，`BASE_URL`/`MODEL` 留空即用代码内置默认值。`EMBEDDING_PROVIDER` 同理用于在 `ark`/`openai`/`tfidf` 三个 embedding Provider 间显式选型；`OPENAI_EMBEDDING_*` 是兼容 OpenAI `/embeddings` 接口格式的通用 Provider 配置（可指向任意中转/自建网关），故意跟上面 LLM 用的 `OPENAI_API_KEY` 等变量区分开，两者可能指向不同网关。以上变量均为**可选**：不配置时系统自动降级为纯规则 + TF-IDF 模式（`NullProvider` + `TfidfProvider`），完整闭环仍能跑通，只是语义分析和召回精度会弱一些。
